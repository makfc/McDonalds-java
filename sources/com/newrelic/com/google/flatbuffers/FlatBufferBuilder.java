...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int initial_size) {
        ...
        this.f6075bb = newByteBuffer(initial_size);
    }
    ...
    public FlatBufferBuilder init(ByteBuffer existing_bb) {
        this.f6075bb = existing_bb;
        this.f6075bb.clear();
        this.f6075bb.order(ByteOrder.LITTLE_ENDIAN);
        this.minalign = 1;
        this.space = this.f6075bb.capacity();
        this.vtable_in_use = 0;
        this.nested = false;
        this.finished = false;
        this.object_start = 0;
    ...
    public int offset() {
        return this.f6075bb.capacity() - this.space;
    }

    public void pad(int byte_size) {
        for (int i = 0; i < byte_size; i++) {
            ByteBuffer byteBuffer = this.f6075bb;
            int i2 = this.space - 1;
            this.space = i2;
    ...
    public void prep(int size, int additional_bytes) {
        ...
        int align_size = ((((this.f6075bb.capacity() - this.space) + additional_bytes) ^ -1) + 1) & (size - 1);
        while (this.space < (align_size + size) + additional_bytes) {
            int old_buf_size = this.f6075bb.capacity();
            this.f6075bb = growByteBuffer(this.f6075bb);
            this.space += this.f6075bb.capacity() - old_buf_size;
        }
        pad(align_size);
    }
    ...
    public ByteBuffer createUnintializedVector(int elem_size, int num_elems, int alignment) {
        ...
        ByteBuffer copy = this.f6075bb.slice().order(ByteOrder.LITTLE_ENDIAN);
        copy.limit(length);
        return copy;
    }
    ...
    public <T extends Table> int createSortedVectorOfTables(T obj, int[] offsets) {
        obj.sortTables(offsets, this.f6075bb);
    ...
    public int endObject() {
        ...
        for (i = 0; i < this.num_vtables; i++) {
            int vt1 = this.f6075bb.capacity() - this.vtables[i];
            int vt2 = this.space;
            short len = this.f6075bb.getShort(vt1);
            if (len == this.f6075bb.getShort(vt2)) {
                short j = (short) 2;
                while (j < len) {
                    if (this.f6075bb.getShort(vt1 + j) == this.f6075bb.getShort(vt2 + j)) {
                        j += 2;
                    }
                }
        ...
        if (existing_vtable != 0) {
            this.space = this.f6075bb.capacity() - vtableloc;
            this.f6075bb.putInt(this.space, existing_vtable - vtableloc);
        } else {
            ...
            iArr[i3] = offset();
            this.f6075bb.putInt(this.f6075bb.capacity() - vtableloc, offset() - vtableloc);
        }
        this.nested = false;
        return vtableloc;
    }
    ...
    public void required(int table, int field) {
        int table_start = this.f6075bb.capacity() - table;
        if (!(this.f6075bb.getShort((table_start - this.f6075bb.getInt(table_start)) + field) != (short) 0)) {
    ...
    public void finish(int root_table) {
        ...
        this.f6075bb.position(this.space);
        this.finished = true;
    }
    ...
    public byte[] sizedByteArray(int start, int length) {
        ...
        this.f6075bb.position(start);
        this.f6075bb.get(array);
        return array;
    }

    public byte[] sizedByteArray() {
        return sizedByteArray(this.space, this.f6075bb.capacity() - this.space);
    }
}

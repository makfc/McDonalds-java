...
public final class QRCodeMultiReader extends QRCodeReader implements MultipleBarcodeReader {
    ...
    private static List<Result> processStructuredAppend(List<Result> results) {
        boolean hasSA = false;
        for (Result resultMetadata : results) {
            if (resultMetadata.getResultMetadata().containsKey(ResultMetadataType.STRUCTURED_APPEND_SEQUENCE)) {
                hasSA = true;
                break;
            }
        }
        ...
        for (Result saResult2 : saResults) {
            ...
            if (saResult2.getResultMetadata().containsKey(ResultMetadataType.BYTE_SEGMENTS)) {
                for (Object segment2 : (Iterable) saResult2.getResultMetadata().get(ResultMetadataType.BYTE_SEGMENTS)) {

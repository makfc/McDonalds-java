...
public final class VCardResultParser extends ResultParser {
    ...
    public AddressBookParsedResult parse(Result result) {
        ...
        if (names == null) {
            names = matchVCardPrefixedField(DCSProfile.INDICATOR_FALSE, rawText, true, false);
            formatNames(names);
        }
        ...
        List<String> title = matchSingleVCardPrefixedField(NativeProtocol.METHOD_ARGS_TITLE, rawText, true, false);
    ...
    private static void maybeAppendFragment(ByteArrayOutputStream fragmentBuffer, String charset, StringBuilder result) {
        if (fragmentBuffer.size() > 0) {
            ...
            if (charset == null) {
                fragment = new String(fragmentBytes, Charset.forName(Utf8Charset.NAME));
            } else {
                try {
    ...
    private static void maybeAppendComponent(String[] components, int i, StringBuilder newName) {
        if (components[i] != null && !components[i].isEmpty()) {
            if (newName.length() > 0) {
                newName.append(SafeJsonPrimitive.NULL_CHAR);
            }

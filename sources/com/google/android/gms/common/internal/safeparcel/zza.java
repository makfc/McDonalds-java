...
public class zza {
    ...
    public static int zzaq(Parcel parcel) {
        int zzap = zzap(parcel);
        int zza = zza(parcel, zzap);
        int dataPosition = parcel.dataPosition();
        if (zzcj(zzap) != 20293) {
            String str = "Expected object header. Got 0x";
            String valueOf = String.valueOf(Integer.toHexString(zzap));
            throw new zza(valueOf.length() != 0 ? str.concat(valueOf) : new String(str), parcel);
        }
        zzap = dataPosition + zza;
        if (zzap >= dataPosition && zzap <= parcel.dataSize()) {
            return zzap;
        }
        throw new zza("Size read is invalid start=" + dataPosition + " end=" + zzap, parcel);
    }

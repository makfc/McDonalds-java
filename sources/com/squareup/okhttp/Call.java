...
public class Call {
    ...
    public Response getResponse(Request request, boolean forWebSocket) throws IOException {
        ...
        if (body != null) {
            ...
            if (contentType != null) {
                requestBuilder.header(TransactionStateUtil.CONTENT_TYPE_HEADER, contentType.toString());
            }
            long contentLength = body.contentLength();
            if (contentLength != -1) {
                requestBuilder.header(TransactionStateUtil.CONTENT_LENGTH_HEADER, Long.toString(contentLength));
                requestBuilder.removeHeader("Transfer-Encoding");
            } else {
                requestBuilder.header("Transfer-Encoding", "chunked");
                requestBuilder.removeHeader(TransactionStateUtil.CONTENT_LENGTH_HEADER);
            }
            request = !(requestBuilder instanceof Builder) ? requestBuilder.build() : OkHttp2Instrumentation.build(requestBuilder);
        }

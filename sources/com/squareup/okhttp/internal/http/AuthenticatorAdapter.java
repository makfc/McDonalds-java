    ...
    public Request authenticate(Proxy proxy, Response response) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ("Basic".equalsIgnoreCase(challenge.getScheme())) {
                ...
                if (auth != null) {
                    Builder header = request.newBuilder().header("Authorization", Credentials.basic(auth.getUserName(), new String(auth.getPassword())));
                    return !(header instanceof Builder) ? header.build() : OkHttp2Instrumentation.build(header);
                }
            }
        }
        return null;
    ...
    public Request authenticateProxy(Proxy proxy, Response response) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ("Basic".equalsIgnoreCase(challenge.getScheme())) {
                ...
                if (auth != null) {
                    Builder header = request.newBuilder().header("Proxy-Authorization", Credentials.basic(auth.getUserName(), new String(auth.getPassword())));
                    return !(header instanceof Builder) ? header.build() : OkHttp2Instrumentation.build(header);
                }
            }
        }

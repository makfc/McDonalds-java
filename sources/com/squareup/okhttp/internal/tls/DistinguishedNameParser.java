...
final class DistinguishedNameParser {
    ...
    private int cur;
    /* renamed from: dn */
    private final String f6704dn;
    private int end;
    private final int length = this.f6704dn.length();
    private int pos;

    public DistinguishedNameParser(X500Principal principal) {
        this.f6704dn = principal.getName("RFC2253");
    }

    private String nextAT() {
        while (this.pos < this.length && this.chars[this.pos] == SafeJsonPrimitive.NULL_CHAR) {
            this.pos++;
        }
        ...
        this.beg = this.pos;
        this.pos++;
        while (this.pos < this.length && this.chars[this.pos] != '=' && this.chars[this.pos] != SafeJsonPrimitive.NULL_CHAR) {
            this.pos++;
        }
        if (this.pos >= this.length) {
            throw new IllegalStateException("Unexpected end of DN: " + this.f6704dn);
        }
        this.end = this.pos;
        if (this.chars[this.pos] == SafeJsonPrimitive.NULL_CHAR) {
            while (this.pos < this.length && this.chars[this.pos] != '=' && this.chars[this.pos] == SafeJsonPrimitive.NULL_CHAR) {
                this.pos++;
            }
        ...
        while (this.pos < this.length) {
            ...
            switch (this.chars[this.pos]) {
                case ' ':
                    ...
                    i = this.end;
                    this.end = i + 1;
                    cArr[i] = SafeJsonPrimitive.NULL_CHAR;
                    while (this.pos < this.length && this.chars[this.pos] == SafeJsonPrimitive.NULL_CHAR) {
                        cArr = this.chars;
                        i = this.end;
                        this.end = i + 1;
    ...
    private int getByte(int position) {
        if (position + 1 >= this.length) {
            throw new IllegalStateException("Malformed DN: " + this.f6704dn);
        }
    ...
    public String findMostSpecific(String attributeType) {
        ...
        this.end = 0;
        this.cur = 0;
        this.chars = this.f6704dn.toCharArray();

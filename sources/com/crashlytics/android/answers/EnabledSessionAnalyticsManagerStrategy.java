...
import p041io.fabric.sdk.android.services.events.TimeBasedFileRollOverRunnable;
...
class EnabledSessionAnalyticsManagerStrategy implements SessionAnalyticsManagerStrategy {
    ...
    public void setAnalyticsSettingsData(AnalyticsSettingsData analyticsSettingsData, String protocolAndHostOverride) {
        ...
        Fabric.getLogger().mo34403d("Answers", "Firebase analytics forwarding " + (this.forwardToFirebaseAnalyticsEnabled ? "enabled" : "disabled"));
        Fabric.getLogger().mo34403d("Answers", "Firebase analytics including purchase events " + (this.includePurchaseEventsInForwardedEvents ? "enabled" : "disabled"));
        this.customEventsEnabled = analyticsSettingsData.trackCustomEvents;
        Fabric.getLogger().mo34403d("Answers", "Custom event tracking " + (this.customEventsEnabled ? "enabled" : "disabled"));
        this.predefinedEventsEnabled = analyticsSettingsData.trackPredefinedEvents;
        Fabric.getLogger().mo34403d("Answers", "Predefined event tracking " + (this.predefinedEventsEnabled ? "enabled" : "disabled"));
        if (analyticsSettingsData.samplingRate > 1) {
            Fabric.getLogger().mo34403d("Answers", "Event sampling enabled");
    ...
    public void processEvent(Builder builder) {
        ...
        if (!this.customEventsEnabled && Type.CUSTOM.equals(event.type)) {
            Fabric.getLogger().mo34403d("Answers", "Custom events tracking disabled - skipping event: " + event);
        } else if (!this.predefinedEventsEnabled && Type.PREDEFINED.equals(event.type)) {
            Fabric.getLogger().mo34403d("Answers", "Predefined events tracking disabled - skipping event: " + event);
        } else if (this.eventFilter.skipEvent(event)) {
            Fabric.getLogger().mo34403d("Answers", "Skipping filtered event: " + event);
        } else {
            try {
                this.filesManager.writeEvent(event);
            } catch (IOException e) {
                Fabric.getLogger().mo34406e("Answers", "Failed to write event: " + event, e);
            }
            ...
            if (!isPurchaseEvent || this.includePurchaseEventsInForwardedEvents) {
                try {
                    ...
                    Fabric.getLogger().mo34406e("Answers", "Failed to map event to Firebase: " + event, e2);
                }
            }
        }
    }

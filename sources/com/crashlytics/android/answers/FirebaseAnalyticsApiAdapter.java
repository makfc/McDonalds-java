...
class FirebaseAnalyticsApiAdapter {
    ...
    public void processEvent(SessionEvent sessionEvent) {
        ...
        if (eventLogger == null) {
            Fabric.getLogger().mo34403d("Answers", "Firebase analytics logging was enabled, but not available...");
            return;
        }
        FirebaseAnalyticsEvent mappedEvent = this.eventMapper.mapEvent(sessionEvent);
        if (mappedEvent == null) {
            Fabric.getLogger().mo34403d("Answers", "Fabric event was not mappable to Firebase event: " + sessionEvent);
            return;
        }

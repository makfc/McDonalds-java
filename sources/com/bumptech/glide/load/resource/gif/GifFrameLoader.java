...
class GifFrameLoader {
    ...
    private static GenericRequestBuilder<GifDecoder, GifDecoder, Bitmap, Bitmap> getRequestBuilder(Context context, GifDecoder gifDecoder, int width, int height, BitmapPool bitmapPool) {
        ...
        return Glide.with(context).using(frameLoader, GifDecoder.class).load(gifDecoder).mo14158as(Bitmap.class).sourceEncoder(NullEncoder.get()).decoder(frameResourceDecoder).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.NONE).override(width, height);
    }
}

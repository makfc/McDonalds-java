...
class DefaultSettingsJsonTransform implements SettingsJsonTransform {
    ...
    public SettingsData buildFromJson(CurrentTimeProvider currentTimeProvider, JSONObject json) throws JSONException {
        ...
        return new SettingsData(getExpiresAtFrom(currentTimeProvider, (long) cacheDuration, json), buildAppDataFrom(json.getJSONObject("app")), buildSessionDataFrom(json.getJSONObject("session")), buildPromptDataFrom(json.getJSONObject("prompt")), buildFeaturesSessionDataFrom(json.getJSONObject("features")), buildAnalyticsSessionDataFrom(json.getJSONObject(Analytics.ANALYTICS_CONFIGS_PATH)), buildBetaSettingsDataFrom(json.getJSONObject("beta")), settingsVersion, cacheDuration);
    }

    private AppSettingsData buildAppDataFrom(JSONObject json) throws JSONException {
        String identifier = json.getString("identifier");
        String status = json.getString("status");
        String url = json.getString(NativeProtocol.IMAGE_URL_KEY);
    ...
    private FeaturesSettingsData buildFeaturesSessionDataFrom(JSONObject json) {
        return new FeaturesSettingsData(json.optBoolean("prompt_enabled", false), json.optBoolean("collect_logged_exceptions", true), json.optBoolean("collect_reports", true), json.optBoolean("collect_analytics", false), json.optBoolean("firebase_crashlytics_enabled", false));
    }

    private AnalyticsSettingsData buildAnalyticsSessionDataFrom(JSONObject json) {
        return new AnalyticsSettingsData(json.optString(NativeProtocol.IMAGE_URL_KEY, "https://e.crashlytics.com/spi/v2/events"), json.optInt("flush_interval_secs", 600), json.optInt("max_byte_size_per_file", 8000), json.optInt("max_file_count_per_send", 1), json.optInt("max_pending_send_file_count", 100), json.optBoolean("forward_to_google_analytics", false), json.optBoolean("include_purchase_events_in_forwarded_events", false), json.optBoolean("track_custom_events", true), json.optBoolean("track_predefined_events", true), json.optInt("sampling_rate", 1), json.optBoolean("flush_on_background", true));
    }
    ...
    private PromptSettingsData buildPromptDataFrom(JSONObject json) throws JSONException {
        return new PromptSettingsData(json.optString(PushConstants.TITLE_KEY, "Send Crash Report?"), json.optString(HexAttributes.HEX_ATTR_MESSAGE, "Looks like we crashed! Please help us fix the problem by sending a crash report."), json.optString("send_button_title", "Send"), json.optBoolean("show_cancel_button", true), json.optString("cancel_button_title", "Don't Send"), json.optBoolean("show_always_send_button", true), json.optString("always_send_button_title", "Always Send"));
    }

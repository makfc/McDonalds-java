...
class AdvertisingInfoProvider {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        ...
        if (isInfoValid(infoToReturn)) {
            Fabric.getLogger().mo34403d("Fabric", "Using AdvertisingInfo from Preference Store");
        ...
        new Thread(new BackgroundPriorityRunnable() {
            public void onRun() {
                ...
                if (!advertisingInfo.equals(infoToStore)) {
                    Fabric.getLogger().mo34403d("Fabric", "Asychronously getting Advertising Info and storing it to preferences");
    ...
    private AdvertisingInfo getAdvertisingInfoFromStrategies() {
        ...
        if (isInfoValid(infoToReturn)) {
            Fabric.getLogger().mo34403d("Fabric", "Using AdvertisingInfo from Reflection Provider");
        } else {
            infoToReturn = getServiceStrategy().getAdvertisingInfo();
            if (isInfoValid(infoToReturn)) {
                Fabric.getLogger().mo34403d("Fabric", "Using AdvertisingInfo from Service Provider");
            } else {
                Fabric.getLogger().mo34403d("Fabric", "AdvertisingInfo not present");
            }
        }
        return infoToReturn;
    }

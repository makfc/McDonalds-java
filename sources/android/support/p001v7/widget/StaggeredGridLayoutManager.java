...
import android.support.p001v7.widget.RecyclerView.LayoutManager.LayoutPrefetchRegistry;
import android.support.p001v7.widget.RecyclerView.LayoutManager.Properties;
...
public class StaggeredGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
    ...
    private final Runnable mCheckForGapsRunnable = new C04181();
    ...
    class AnchorInfo {
        ...
        public void saveSpanReferenceLines(Span[] spans) {
            ...
            for (int i = 0; i < spanCount; i++) {
                this.mSpanReferenceLines[i] = spans[i].getStartLine(ExploreByTouchHelper.INVALID_ID);
            }
        }
    ...
    public static class LayoutParams extends android.support.p001v7.widget.RecyclerView.LayoutParams {
        boolean mFullSpan;
        Span mSpan;
    ...
    static class LazySpanLookup {
        ...
        static class FullSpanItem implements Parcelable {
            public static final Creator<FullSpanItem> CREATOR = new C04191();
            int mGapDir;
            int[] mGapPerSpan;
            ...
            static class C04191 implements Creator<FullSpanItem> {
                C04191() {
                }
    ...
    public static class SavedState implements Parcelable {
        public static final Creator<SavedState> CREATOR = new C04201();
        ...
        static class C04201 implements Creator<SavedState> {
            C04201() {
            }
    ...
    class Span {
        ...
        public int getStartLine(int def) {
            if (this.mCachedStart != ExploreByTouchHelper.INVALID_ID) {
        ...
        public int getEndLine(int def) {
            if (this.mCachedEnd != ExploreByTouchHelper.INVALID_ID) {
        ...
        public void cacheReferenceLineAndClear(boolean reverseLayout, int offset) {
            int reference;
            if (reverseLayout) {
                reference = getEndLine(ExploreByTouchHelper.INVALID_ID);
            } else {
                reference = getStartLine(ExploreByTouchHelper.INVALID_ID);
            }
            clear();
            if (reference != ExploreByTouchHelper.INVALID_ID) {
        ...
        if (getChildCount() > 0 && (this.mPendingSavedState == null || this.mPendingSavedState.mSpanOffsetsSize < 1)) {
            ...
            if (anchorInfo.mInvalidateOffsets) {
                for (i = 0; i < this.mSpanCount; i++) {
                    this.mSpans[i].clear();
                    if (anchorInfo.mOffset != ExploreByTouchHelper.INVALID_ID) {
    ...
    private void repositionToWrapContentIfNecessary() {
        if (this.mSecondaryOrientation.getMode() != 1073741824) {
            ...
            if (this.mSecondaryOrientation.getMode() == ExploreByTouchHelper.INVALID_ID) {
        ...
        if (this.mPendingSavedState.mSpanOffsetsSize > 0) {
            if (this.mPendingSavedState.mSpanOffsetsSize == this.mSpanCount) {
                for (int i = 0; i < this.mSpanCount; i++) {
                    ...
                    if (line != ExploreByTouchHelper.INVALID_ID) {
    ...
    public boolean updateAnchorFromPendingData(State state, AnchorInfo anchorInfo) {
        ...
        if (this.mPendingScrollPosition < 0 || this.mPendingScrollPosition >= state.getItemCount()) {
            ...
            if (child != null) {
                ...
                if (this.mPendingScrollPositionOffset != ExploreByTouchHelper.INVALID_ID) {
                    if (anchorInfo.mLayoutFromEnd) {
    ...
    private int updateSpecWithExtra(int spec, int startInset, int endInset) {
        ...
        if (mode == ExploreByTouchHelper.INVALID_ID || mode == 1073741824) {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            for (int i = 0; i < this.mSpanCount; i++) {
                int line;
                if (this.mLastLayoutFromEnd) {
                    line = this.mSpans[i].getEndLine(ExploreByTouchHelper.INVALID_ID);
                    if (line != ExploreByTouchHelper.INVALID_ID) {
                        line -= this.mPrimaryOrientation.getEndAfterPadding();
                    }
                } else {
                    line = this.mSpans[i].getStartLine(ExploreByTouchHelper.INVALID_ID);
                    if (line != ExploreByTouchHelper.INVALID_ID) {
                        line -= this.mPrimaryOrientation.getStartAfterPadding();
                    }
                }
    ...
    private void fixEndGap(Recycler recycler, State state, boolean canOffsetChildren) {
        int maxEndLine = getMaxEnd(ExploreByTouchHelper.INVALID_ID);
        if (maxEndLine != ExploreByTouchHelper.INVALID_ID) {
    ...
    private int fill(Recycler recycler, LayoutState layoutState, State state) {
        ...
        if (this.mLayoutState.mInfinite) {
            if (layoutState.mLayoutDirection == 1) {
                ...
                targetLine = ExploreByTouchHelper.INVALID_ID;
            }
    ...
    public boolean areAllEndsEqual() {
        int end = this.mSpans[0].getEndLine(ExploreByTouchHelper.INVALID_ID);
        for (int i = 1; i < this.mSpanCount; i++) {
            if (this.mSpans[i].getEndLine(ExploreByTouchHelper.INVALID_ID) != end) {
                return false;
            }
        }
        return true;
    ...
    public boolean areAllStartsEqual() {
        int start = this.mSpans[0].getStartLine(ExploreByTouchHelper.INVALID_ID);
        for (int i = 1; i < this.mSpanCount; i++) {
            if (this.mSpans[i].getStartLine(ExploreByTouchHelper.INVALID_ID) != start) {
                return false;
            }
        }
        return true;
    ...
    private Span getNextSpan(LayoutState layoutState) {
        ...
        Span max = null;
        int maxLine = ExploreByTouchHelper.INVALID_ID;
    ...
    public View onFocusSearchFailed(View focused, int direction, Recycler recycler, State state) {
        ...
        if (layoutDir == ExploreByTouchHelper.INVALID_ID) {
            return null;
        }
        int referenceChildPosition;
        View view;
    ...
    private int convertFocusDirectionToLayoutDirection(int focusDirection) {
        ...
        switch (focusDirection) {
            ...
            case 2:
                ...
                return 1;
            case 17:
                return this.mOrientation != 0 ? ExploreByTouchHelper.INVALID_ID : -1;
            case 33:
                return this.mOrientation != 1 ? ExploreByTouchHelper.INVALID_ID : -1;
            case 66:
                if (this.mOrientation != 0) {

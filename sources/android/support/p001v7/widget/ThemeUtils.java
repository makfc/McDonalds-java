...
class ThemeUtils {
    ...
    public static int getDisabledThemeAttrColor(Context context, int attr) {
        ColorStateList csl = ThemeUtils.getThemeAttrColorStateList(context, attr);
        if (csl != null && csl.isStateful()) {
            return csl.getColorForState(DISABLED_STATE_SET, csl.getDefaultColor());
        }
        TypedValue tv = ThemeUtils.getTypedValue();
        context.getTheme().resolveAttribute(16842803, tv, true);
        return ThemeUtils.getThemeAttrColor(context, attr, tv.getFloat());
    }
    ...
    static int getThemeAttrColor(Context context, int attr, float alpha) {
        int color = ThemeUtils.getThemeAttrColor(context, attr);

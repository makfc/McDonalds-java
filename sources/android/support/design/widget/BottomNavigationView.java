...
public class BottomNavigationView extends FrameLayout {
    ...
    static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C00831());
        Bundle menuPresenterState;

        /* renamed from: android.support.design.widget.BottomNavigationView$SavedState$1 */
        static class C00831 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00831() {
            }
    ...
    public BottomNavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, C0045R.styleable.BottomNavigationView, defStyleAttr, C0045R.style.Widget_Design_BottomNavigationView);
        if (a.hasValue(C0045R.styleable.BottomNavigationView_itemIconTint)) {
            this.mMenuView.setIconTintList(a.getColorStateList(C0045R.styleable.BottomNavigationView_itemIconTint));
        } else {
            this.mMenuView.setIconTintList(createDefaultColorStateList(16842808));
        }
        if (a.hasValue(C0045R.styleable.BottomNavigationView_itemTextColor)) {
            this.mMenuView.setItemTextColor(a.getColorStateList(C0045R.styleable.BottomNavigationView_itemTextColor));
        } else {
            this.mMenuView.setItemTextColor(createDefaultColorStateList(16842808));
        }
        if (a.hasValue(C0045R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0045R.styleable.BottomNavigationView_elevation, 0));
        }
        this.mMenuView.setItemBackgroundRes(a.getResourceId(C0045R.styleable.BottomNavigationView_itemBackground, 0));
        if (a.hasValue(C0045R.styleable.BottomNavigationView_menu)) {
            inflateMenu(a.getResourceId(C0045R.styleable.BottomNavigationView_menu, 0));
        }
        a.recycle();
        ...
        this.mMenu.setCallback(new C00821());
    }
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        divider.setBackgroundColor(ContextCompat.getColor(context, C0045R.color.design_bottom_navigation_shadow_color));
        divider.setLayoutParams(new LayoutParams(-1, getResources().getDimensionPixelSize(C0045R.dimen.design_bottom_navigation_shadow_height)));
        addView(divider);
    }

...
class DocumentsContractApi19 {
    ...
    public static boolean isVirtual(Context context, Uri self) {
        if (DocumentsContractApi19.isDocumentUri(context, self) && (DocumentsContractApi19.getFlags(context, self) & 512) != 0) {
            return true;
        }
        return false;
    }
    ...
    public static long getFlags(Context context, Uri self) {
        return DocumentsContractApi19.queryForLong(context, self, "flags", 0);
    }
    ...
    public static long lastModified(Context context, Uri self) {
        return DocumentsContractApi19.queryForLong(context, self, "last_modified", 0);
    }

    public static long length(Context context, Uri self) {
        return DocumentsContractApi19.queryForLong(context, self, "_size", 0);
    }

    public static boolean canRead(Context context, Uri self) {
        if (context.checkCallingOrSelfUriPermission(self, 1) == 0 && !TextUtils.isEmpty(DocumentsContractApi19.getRawType(context, self))) {
            return true;
        }
        return false;
    }
    ...
    private static int queryForInt(Context context, Uri self, String column, int defaultValue) {
        return (int) DocumentsContractApi19.queryForLong(context, self, column, (long) defaultValue);
    }

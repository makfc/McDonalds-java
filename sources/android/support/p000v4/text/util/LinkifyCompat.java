...
public final class LinkifyCompat {
    private static final Comparator<LinkSpec> COMPARATOR = new C02551();
    private static final String[] EMPTY_STRING = new String[0];

    /* renamed from: android.support.v4.text.util.LinkifyCompat$1 */
    static class C02551 implements Comparator<LinkSpec> {
        C02551() {
        }
    ...
    public static final boolean addLinks(@NonNull Spannable text, int mask) {
        ...
        if ((mask & 1) != 0) {
            ...
            LinkifyCompat.gatherLinks(links, spannable, PatternsCompat.AUTOLINK_WEB_URL, new String[]{"http://", "https://", "rtsp://"}, Linkify.sUrlMatchFilter, null);
        }
        if ((mask & 2) != 0) {
    ...
    public static final boolean addLinks(@NonNull TextView text, int mask) {
        ...
        if (!(t instanceof Spannable)) {
            ...
            text.setText(s);
            return true;
        } else if (!LinkifyCompat.addLinks((Spannable) t, mask)) {
            return false;
        } else {
            LinkifyCompat.addLinkMovementMethod(text);
            return true;
    ...
    public static final void addLinks(@NonNull TextView text, @NonNull Pattern pattern, @Nullable String defaultScheme, @Nullable String[] schemes, @Nullable MatchFilter matchFilter, @Nullable TransformFilter transformFilter) {
        ...
        if (LinkifyCompat.addLinks(spannable, pattern, defaultScheme, schemes, matchFilter, transformFilter)) {
    ...
    public static final boolean addLinks(@NonNull Spannable spannable, @NonNull Pattern pattern, @Nullable String defaultScheme, @Nullable String[] schemes, @Nullable MatchFilter matchFilter, @Nullable TransformFilter transformFilter) {
        ...
        while (m.find()) {
            ...
            if (allowed) {
                LinkifyCompat.applyLink(LinkifyCompat.makeUrl(m.group(0), schemesCopy, m, transformFilter), start, end, spannable);
                hasMatches = true;
            }
        }
        return hasMatches;
    ...
    private static void gatherLinks(ArrayList<LinkSpec> links, Spannable s, Pattern pattern, String[] schemes, MatchFilter matchFilter, TransformFilter transformFilter) {
        ...
        while (m.find()) {
            ...
            if (matchFilter == null || matchFilter.acceptMatch(s, start, end)) {
                ...
                spec.url = LinkifyCompat.makeUrl(m.group(0), schemes, m, transformFilter);
                spec.start = start;
                spec.end = end;
                links.add(spec);
            }
            ...
            try {
                ...
                if (address != null) {
                    ...
                    if (start >= 0) {
                        ...
                        base += end;
                        try {
                            spec.url = "geo:0,0?q=" + URLEncoder.encode(address, Utf8Charset.NAME);
                            links.add(spec);

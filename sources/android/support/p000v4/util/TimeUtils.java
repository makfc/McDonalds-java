...
public final class TimeUtils {
    ...
    private static int formatDurationLocked(long duration, int fieldLen) {
        ...
        if (duration == 0) {
            fieldLen--;
            while (0 < fieldLen) {
                formatStr[0] = SafeJsonPrimitive.NULL_CHAR;
            }
            formatStr[0] = '0';
            return 1;
        }
        ...
        int pos = 0;
        if (fieldLen != 0) {
            int myLen = TimeUtils.accumField(days, 1, false, 0);
            myLen += TimeUtils.accumField(hours, 1, myLen > 0, 2);
            myLen += TimeUtils.accumField(minutes, 1, myLen > 0, 2);
            myLen += TimeUtils.accumField(seconds, 1, myLen > 0, 2);
            for (myLen += TimeUtils.accumField(millis, 2, true, myLen > 0 ? 3 : 0) + 1; myLen < fieldLen; myLen++) {
                formatStr[pos] = SafeJsonPrimitive.NULL_CHAR;
                pos++;
            }
        }
        ...
        pos = TimeUtils.printField(formatStr, millis, 'm', pos, true, i);
        formatStr[pos] = 's';
        return pos + 1;
    }
    ...
    public static void formatDuration(long duration, StringBuilder builder) {
        synchronized (sFormatSync) {
            builder.append(sFormatStr, 0, TimeUtils.formatDurationLocked(duration, 0));
        }
    }

    public static void formatDuration(long duration, PrintWriter pw, int fieldLen) {
        synchronized (sFormatSync) {
            pw.print(new String(sFormatStr, 0, TimeUtils.formatDurationLocked(duration, fieldLen)));
        }
    }

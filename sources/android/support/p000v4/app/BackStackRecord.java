...
import android.support.p000v4.app.FragmentManagerImpl.OpGenerator;
...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, OpGenerator {
    ...
    String mName;
    ArrayList<C0153Op> mOps = new ArrayList();
    int mPopEnterAnim;
    int mPopExitAnim;
    ...
    public void dump(String prefix, PrintWriter writer, boolean full) {
        ...
        if (!this.mOps.isEmpty()) {
            ...
            for (int opNum = 0; opNum < numOps; opNum++) {
                String cmdStr;
                C0153Op op = (C0153Op) this.mOps.get(opNum);
                switch (op.cmd) {
                    case 0:
                        cmdStr = "NULL";
                        break;
    ...
    public boolean interactsWith(int containerId) {
        int numOps = this.mOps.size();
        for (int opNum = 0; opNum < numOps; opNum++) {
            if (((C0153Op) this.mOps.get(opNum)).fragment.mContainerId == containerId) {
                return true;
            }
        }
        return false;
    ...
    public boolean interactsWith(ArrayList<BackStackRecord> records, int startIndex, int endIndex) {
        ...
        int lastContainer = -1;
        for (int opNum = 0; opNum < numOps; opNum++) {
            int container = ((C0153Op) this.mOps.get(opNum)).fragment.mContainerId;
            if (!(container == 0 || container == lastContainer)) {
                ...
                for (int i = startIndex; i < endIndex; i++) {
                    ...
                    for (int thoseOpIndex = 0; thoseOpIndex < numThoseOps; thoseOpIndex++) {
                        if (((C0153Op) record.mOps.get(thoseOpIndex)).fragment.mContainerId == container) {
                            return true;
                        }
                    }
                }
    ...
    public boolean isPostponed() {
        for (int opNum = 0; opNum < this.mOps.size(); opNum++) {
            if (BackStackRecord.isFragmentPostponed((C0153Op) this.mOps.get(opNum))) {
                return true;
            }
        }
        return false;
    ...
    private static boolean isFragmentPostponed(C0153Op op) {
        Fragment fragment = op.fragment;

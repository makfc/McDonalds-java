...
import android.support.p000v4.app.BackStackRecord.C0153Op;
...
class FragmentTransition {
        ...
        if (fragmentManager.mCurState >= 1 && VERSION.SDK_INT >= 21) {
            ...
            for (i = startIndex; i < endIndex; i++) {
                ...
                if (((Boolean) isRecordPop.get(i)).booleanValue()) {
                    FragmentTransition.calculatePopFragments(record, transitioningFragments, isOptimized);
                } else {
                    FragmentTransition.calculateFragments(record, transitioningFragments, isOptimized);
                }
            }
    ...
    private static Object configureSharedElementsOptimized(ViewGroup sceneRoot, View nonExistentView, ArrayMap<String, String> nameOverrides, FragmentContainerTransition fragments, ArrayList<View> sharedElementsOut, ArrayList<View> sharedElementsIn, Object enterTransition, Object exitTransition) {
        ...
        if (nameOverrides.isEmpty()) {
            ...
            sharedElementTransition = FragmentTransition.getSharedElementTransition(inFragment, outFragment, inIsPop);
        }
        ...
        if (sharedElementTransition != null) {
            ...
            FragmentTransition.setOutEpicenter(sharedElementTransition, exitTransition, outSharedElements, fragments.firstOutIsPop, fragments.firstOutTransaction);
            epicenter = new Rect();
    ...
    private static Object configureSharedElementsUnoptimized(ViewGroup sceneRoot, View nonExistentView, ArrayMap<String, String> nameOverrides, FragmentContainerTransition fragments, ArrayList<View> sharedElementsOut, ArrayList<View> sharedElementsIn, Object enterTransition, Object exitTransition) {
        ...
        if (sharedElementTransition2 != null) {
            ...
            FragmentTransition.setOutEpicenter(sharedElementTransition2, exitTransition, outSharedElements, fragments.firstOutIsPop, fragments.firstOutTransaction);
    ...
    public static void calculateFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isOptimized) {
        ...
        for (int opNum = 0; opNum < numOps; opNum++) {
            FragmentTransition.addToFirstInLastOut(transaction, (C0153Op) transaction.mOps.get(opNum), transitioningFragments, false, isOptimized);
        }
    }
    ...
    public static void calculatePopFragments(BackStackRecord transaction, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isOptimized) {
        if (transaction.mManager.mContainer.onHasView()) {
            for (int opNum = transaction.mOps.size() - 1; opNum >= 0; opNum--) {
                FragmentTransition.addToFirstInLastOut(transaction, (C0153Op) transaction.mOps.get(opNum), transitioningFragments, true, isOptimized);
            }
        }
    }

    private static void addToFirstInLastOut(BackStackRecord transaction, C0153Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isOptimizedTransaction) {

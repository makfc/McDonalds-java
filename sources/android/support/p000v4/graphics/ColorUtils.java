...
public final class ColorUtils {
    ...
    public static int compositeColors(@ColorInt int foreground, @ColorInt int background) {
        ...
        return Color.argb(a, ColorUtils.compositeComponent(Color.red(foreground), fgAlpha, Color.red(background), bgAlpha, a), ColorUtils.compositeComponent(Color.green(foreground), fgAlpha, Color.green(background), bgAlpha, a), ColorUtils.compositeComponent(Color.blue(foreground), fgAlpha, Color.blue(background), bgAlpha, a));
    }
    ...
    public static double calculateContrast(@ColorInt int foreground, @ColorInt int background) {
        ...
        double luminance1 = ColorUtils.calculateLuminance(foreground) + 0.05d;
        double luminance2 = ColorUtils.calculateLuminance(background) + 0.05d;
    ...
    public static int calculateMinimumAlpha(@ColorInt int foreground, @ColorInt int background, float minContrastRatio) {
        if (Color.alpha(background) != 255) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        } else if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(foreground, 255), background) < ((double) minContrastRatio)) {
            return -1;
        } else {
            int minAlpha = 0;
            int maxAlpha = 255;
            for (int numIterations = 0; numIterations <= 10 && maxAlpha - minAlpha > 1; numIterations++) {
                ...
                if (ColorUtils.calculateContrast(ColorUtils.setAlphaComponent(foreground, testAlpha), background) < ((double) minContrastRatio)) {
                    minAlpha = testAlpha;
                } else {
                    maxAlpha = testAlpha;
                }
    ...
    public static void RGBToHSL(@IntRange int r, @IntRange int g, @IntRange int b, @NonNull float[] outHsl) {
        float s;
        float h;
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        return Color.rgb(ColorUtils.constrain(r, 0, 255), ColorUtils.constrain(g, 0, 255), ColorUtils.constrain(b, 0, 255));
    }

    @ColorInt
    public static int setAlphaComponent(@ColorInt int color, @IntRange int alpha) {
    ...
    public static void RGBToLAB(@IntRange int r, @IntRange int g, @IntRange int b, @NonNull double[] outLab) {
    ...
    public static void RGBToXYZ(@IntRange int r, @IntRange int g, @IntRange int b, @NonNull double[] outXyz) {
        if (outXyz.length != 3) {
    ...
    public static void XYZToLAB(@FloatRange double x, @FloatRange double y, @FloatRange double z, @NonNull double[] outLab) {
        if (outLab.length != 3) {
            throw new IllegalArgumentException("outLab must have a length of 3.");
        }
        x = ColorUtils.pivotXyzComponent(x / XYZ_WHITE_REFERENCE_X);
        y = ColorUtils.pivotXyzComponent(y / XYZ_WHITE_REFERENCE_Y);
        z = ColorUtils.pivotXyzComponent(z / XYZ_WHITE_REFERENCE_Z);
        outLab[0] = Math.max(0.0d, (116.0d * y) - 16.0d);
        outLab[1] = 500.0d * (x - y);
        outLab[2] = 200.0d * (y - z);
    }

    public static void LABToXYZ(@FloatRange double l, @FloatRange double a, @FloatRange double b, @NonNull double[] outXyz) {
    ...
    @ColorInt
    public static int XYZToColor(@FloatRange double x, @FloatRange double y, @FloatRange double z) {
        ...
        return Color.rgb(ColorUtils.constrain((int) Math.round(255.0d * (r > 0.0031308d ? (1.055d * Math.pow(r, 0.4166666666666667d)) - 0.055d : r * 12.92d)), 0, 255), ColorUtils.constrain((int) Math.round(255.0d * (g > 0.0031308d ? (1.055d * Math.pow(g, 0.4166666666666667d)) - 0.055d : g * 12.92d)), 0, 255), ColorUtils.constrain((int) Math.round(255.0d * (b > 0.0031308d ? (1.055d * Math.pow(b, 0.4166666666666667d)) - 0.055d : b * 12.92d)), 0, 255));
    }

    @ColorInt
    public static int LABToColor(@FloatRange double l, @FloatRange double a, @FloatRange double b) {
        double[] result = ColorUtils.getTempDouble3Array();
    ...
    @ColorInt
    public static int blendARGB(@ColorInt int color1, @ColorInt int color2, @FloatRange float ratio) {
    ...
    public static void blendHSL(@NonNull float[] hsl1, @NonNull float[] hsl2, @FloatRange float ratio, @NonNull float[] outResult) {
    ...
    public static void blendLAB(@NonNull double[] lab1, @NonNull double[] lab2, @FloatRange double ratio, @NonNull double[] outResult) {
        if (outResult.length != 3) {

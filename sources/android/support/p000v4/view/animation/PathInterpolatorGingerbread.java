...
class PathInterpolatorGingerbread implements Interpolator {
    ...
    public PathInterpolatorGingerbread(Path path) {
        ...
        this.f10mX = new float[numPoints];
        this.f11mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
            this.f10mX[i] = position[0];
            this.f11mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f10mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f10mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f10mX[endIndex] - this.f10mX[startIndex];
        if (xRange == 0.0f) {
            return this.f11mY[startIndex];
        }
        float fraction = (t - this.f10mX[startIndex]) / xRange;
        float startY = this.f11mY[startIndex];
        return ((this.f11mY[endIndex] - startY) * fraction) + startY;
    }

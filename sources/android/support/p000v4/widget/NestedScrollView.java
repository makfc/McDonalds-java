...
public class NestedScrollView extends FrameLayout implements NestedScrollingChild, NestedScrollingParent, ScrollingView {
    ...
    static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = new C02891();
        public int scrollPosition;

        /* renamed from: android.support.v4.widget.NestedScrollView$SavedState$1 */
        static class C02891 implements Creator<SavedState> {
            C02891() {
            }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mFillViewport && MeasureSpec.getMode(heightMeasureSpec) != 0 && getChildCount() > 0) {
            ...
            if (child.getMeasuredHeight() < height) {
                child.measure(NestedScrollView.getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), ((FrameLayout.LayoutParams) child.getLayoutParams()).width), MeasureSpec.makeMeasureSpec((height - getPaddingTop()) - getPaddingBottom(), 1073741824));
            }
        }
    }
    ...
    public void measureChild(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
        child.measure(NestedScrollView.getChildMeasureSpec(parentWidthMeasureSpec, getPaddingLeft() + getPaddingRight(), child.getLayoutParams().width), MeasureSpec.makeMeasureSpec(0, 0));
    }

    /* Access modifiers changed, original: protected */
    public void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        MarginLayoutParams lp = (MarginLayoutParams) child.getLayoutParams();
        child.measure(NestedScrollView.getChildMeasureSpec(parentWidthMeasureSpec, (((getPaddingLeft() + getPaddingRight()) + lp.leftMargin) + lp.rightMargin) + widthUsed, lp.width), MeasureSpec.makeMeasureSpec(lp.topMargin + lp.bottomMargin, 0));
    }
    ...
    private static boolean isViewDescendantOf(View child, View parent) {
        ...
        if ((theParent instanceof ViewGroup) && NestedScrollView.isViewDescendantOf((View) theParent, parent)) {
            return true;
        }
        return false;
    }
    ...
    public void scrollTo(int x, int y) {
        if (getChildCount() > 0) {
            View child = getChildAt(0);
            x = NestedScrollView.clamp(x, (getWidth() - getPaddingRight()) - getPaddingLeft(), child.getWidth());
            y = NestedScrollView.clamp(y, (getHeight() - getPaddingBottom()) - getPaddingTop(), child.getHeight());

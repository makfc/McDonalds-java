...
class FocusStrategy {
    ...
    public static <L, T> T findNextFocusInAbsoluteDirection(@NonNull L focusables, @NonNull CollectionAdapter<L, T> collectionAdapter, @NonNull BoundsAdapter<T> adapter, @Nullable T focused, @NonNull Rect focusedRect, int direction) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (focusable != focused) {
                ...
                if (FocusStrategy.isBetterCandidate(direction, focusedRect, focusableRect, bestCandidateRect)) {
    ...
    private static boolean isBetterCandidate(int direction, @NonNull Rect source, @NonNull Rect candidate, @NonNull Rect currentBest) {
        ...
        if (!FocusStrategy.isCandidate(source, currentBest, direction) || FocusStrategy.beamBeats(direction, source, candidate, currentBest)) {
            return true;
        }
        if (FocusStrategy.beamBeats(direction, source, currentBest, candidate)) {
            return false;
        }
        if (FocusStrategy.getWeightedDistanceFor(FocusStrategy.majorAxisDistance(direction, source, candidate), FocusStrategy.minorAxisDistance(direction, source, candidate)) >= FocusStrategy.getWeightedDistanceFor(FocusStrategy.majorAxisDistance(direction, source, currentBest), FocusStrategy.minorAxisDistance(direction, source, currentBest))) {
            return false;
        }
        return true;
    }
    ...
    private static boolean beamBeats(int direction, @NonNull Rect source, @NonNull Rect rect1, @NonNull Rect rect2) {
        ...
        if (FocusStrategy.beamsOverlap(direction, source, rect2) || !rect1InSrcBeam) {
            return false;
        }
        if (!FocusStrategy.isToDirectionOf(direction, source, rect2) || direction == 17 || direction == 66 || FocusStrategy.majorAxisDistance(direction, source, rect1) < FocusStrategy.majorAxisDistanceToFarEdge(direction, source, rect2)) {
            return true;
        }
        return false;
    }
